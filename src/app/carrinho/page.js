...
export default function Cart() {
 ...
 const [cookies] = useCookies(['clienteId']);
 ...
 const moverProdFav = async (id) => {
   try {
     ...
     const produtoFavorito = favoritos.find((fav) => fav.id == id);
     ...
     if (produtoFavorito) {
       ...
       const addFavoritoResponse = await adicionarProdutoFavoritos(id);
           ...
           <tbody className={style.carrinhoTbody}>
             {(cart.map((item, index) => (
               <tr className={style.carrinhoTr} key={item.id}>
                 ...
                 <td className={style.carrinhoTd}>
                   <button className={style.listaFavBnt} onClick={() => openModal(item.produto_id)}>
                     <TbHeartPlus />
                   </button>
                 </td>
               </tr>
